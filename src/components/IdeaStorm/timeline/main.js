...
const timelinejs = () => {
  ...
  function initTimeline(timelines) {
    timelines.each(function() {
      ...
      timelineComponents.timelineWrapper = timeline.find(
        '.ideastormevents-wrapper'
      )
      timelineComponents.ideastormeventsWrapper = timelineComponents.timelineWrapper.children(
        '.ideastormevents'
      )
      timelineComponents.fillingLine = timelineComponents.ideastormeventsWrapper.children(
        '.filling-line'
      )
      timelineComponents.timelineideastormevents = timelineComponents.ideastormeventsWrapper.find(
        'a'
      )
      timelineComponents.timelineDates = parseDate(
        timelineComponents.timelineideastormevents
      )
      timelineComponents.ideastormeventsMinLapse = minLapse(
        timelineComponents.timelineDates
      )
      timelineComponents.timelineNavigation = timeline.find(
        '.cd-timeline-navigation'
      )
      timelineComponents.ideastormeventsContent = timeline.children(
        '.ideastormevents-content'
      )
      ...
      timelineComponents.ideastormeventsWrapper.on('click', 'a', function(
        event
      ) {
        event.preventDefault()
  ...
  function showNewContent(timelineComponents, timelineTotWidth, string) {
    ...
    const visibleContent = timelineComponents.ideastormeventsContent.find(
      '.selected'
    )
    ...
    if (newContent.length > 0) {
      ...
      const selectedDate = timelineComponents.ideastormeventsWrapper.find(
        '.selected'
      )
  ...
  function updateTimelinePosition(string, event, timelineComponents) {
    ...
    const eventLeft = Number(
      ideastormeventstyle.getPropertyValue('left').replace('px', '')
    )
    ...
    const timelineTotWidth = Number(
      timelineComponents.ideastormeventsWrapper.css('width').replace('px', '')
    )
    const timelineTranslate = getTranslateValue(
      timelineComponents.ideastormeventsWrapper
    )
  ...
  function translateTimeline(timelineComponents, value, totWidth) {
    const ideastormeventsWrapper = timelineComponents.ideastormeventsWrapper.get(
      0
    )
  ...
  function updateFilling(selectedEvent, filling, totWidth) {
    ...
    const ideastormeventstyle = window.getComputedStyle(
      selectedEvent.get(0),
      null
    )
  ...
  function setDatePosition(timelineComponents, min) {
    for (let i = 0; i < timelineComponents.timelineDates.length; i++) {
      ...
      const distanceNorm =
        Math.round(distance / timelineComponents.ideastormeventsMinLapse) + 2
      timelineComponents.timelineideastormevents
        .eq(i)
  ...
  function setTimelineWidth(timelineComponents, width) {
    ...
    updateFilling(
      timelineComponents.ideastormeventsWrapper.find('a.selected'),
  ...
  function updateVisibleContent(event, ideastormeventsContent) {
    ...
    const selectedContent = ideastormeventsContent.find(
      `[data-date="${eventDate}"]`
    )
